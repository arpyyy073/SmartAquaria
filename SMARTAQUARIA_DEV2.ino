...
// AP mode credentials
const char* ap_ssid = "SmartAquaria_Setup";
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("\n\nStarting Smart Aquaria System - Device 2");
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    // Format MAC address for Device ID
...
// The following functions are from your original Device 2 code
void readTemperature() {
  ...
  if (abs(ph_act - lastPHLevel) > 0.1) {
    if (Firebase.ready()) {
      ...
      if (Firebase.updateNode(firebaseData, "/devices/" + deviceID + "/ph", json)) {
        ...
        Serial.print("Failed to send pH value: ");
        Serial.println(firebaseData.errorReason());
      }
    }
  }
...
// Control pumps based on Firebase data
void updatePumpState() {
  if (Firebase.ready()) {
    // Control Pump 1
    if (Firebase.getBool(firebaseData, "/devices/" + deviceID + "/pumps/pump1")) {
      pump1State = firebaseData.boolData();
      digitalWrite(RELAY1_PIN, pump1State ? LOW : HIGH);
      Serial.print("Pump 1 state: ");
      Serial.println(pump1State ? "ON" : "OFF");
    }

    // Control Pump 2
    if (Firebase.getBool(firebaseData, "/devices/" + deviceID + "/pumps/pump2")) {
      pump2State = firebaseData.boolData();
      digitalWrite(RELAY2_PIN, pump2State ? LOW : HIGH);
